...
async function processFile(
  ...
  const state = stateCfg(cfg);
  const driveId = cfg.graph.driveId;
  ...
  await retryWithBackoff(
    async () => {
      ...
      if (status >= 200 && status < 300) {
        ...
        ctx.log(`Uploaded ${kind} for item ${item.id} (${item.name}) → ${status}`);
        // Registry write only after a 2xx from ROOTKey — never optimistically.
...
// from OneDrive edit → ROOTKey ingestion is roughly (webhook delay) OR
// (time to next hour), whichever comes first.
