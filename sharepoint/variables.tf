...
variable "graph_tenant_id" {
  type        = string
  description = "Azure Entra ID tenant ID (a UUID) of the Microsoft 365 tenant whose SharePoint site will be monitored."
...
variable "graph_client_id" {
  type        = string
  description = "Application (client) ID of the pre-existing App Registration used to call Microsoft Graph. Requires Sites.Read.All and Files.Read.All application permissions with admin consent."
...
variable "site_url" {
  type        = string
  description = "Full URL of the SharePoint site to monitor (e.g. https://contoso.sharepoint.com/sites/legal). The Function App resolves this to a site ID at runtime, then discovers every document library (drive) on the site and subscribes to each."

  validation {
    condition     = can(regex("^https://[a-zA-Z0-9-]+\\.sharepoint\\.com(/.*)?$", var.site_url))
    error_message = "site_url must be a SharePoint site URL, e.g. https://contoso.sharepoint.com/sites/legal."
  }
}
...
variable "tags" {
  type        = map(string)
  default     = {}
  description = "Extra tags applied to every resource created by this module. Module-managed tags (rootkey:managed-by, rootkey:connector, rootkey:site-url) cannot be overridden."
}
