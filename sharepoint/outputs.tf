...
output "notification_url" {
  description = "Microsoft Graph webhook notification URL. Used by the Function App itself to register the subscriptions; surfaced for diagnostics."
...
output "storage_account_name" {
  description = "Storage Account used for Function App backing, per-drive delta state, and the DLQ."
  value       = azurerm_storage_account.func.name
}

output "dlq_queue_name" {
  description = "Storage Queue receiving per-file failures after retries — monitor this for missed uploads."
